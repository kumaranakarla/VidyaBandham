...
// year filter bolted onto the existing TET Prep page — the plan is for this
// specific content to move behind a paid subscription down the road, and
// keeping it as its own page from the start means a route guard (e.g.
// `canActivate: [subscriptionGuard]`) can be added later without touching
// TET Prep or Mock Test at all.
@Component({
  ...
  template: `
    ...
    <p class="intro">
      ...
      Department of School Education's own results portal. Currently: Paper 2A (Maths & Science), both August 2026
      shifts.
    </p>
    ...
    <p *ngIf="!loading && questions.length === 0">No 2026 questions added yet.</p>
...
export class Tet2026Component implements OnInit {
  ...
  // Lets a user isolate one shift's paper (e.g. just Shift 1) instead of both at once.
  get sources(): string[] {
    return Array.from(new Set(this.questions.map((q) => q.source).filter((s): s is string => !!s)));
  }
